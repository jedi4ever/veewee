      ...
      module BoxCommand
        ...
        def create_disk
          ...
          if !found
            ...
            command ="#{@vboxcmd} createhd --filename '#{File.join(File.new(place).path,name,name+"."+definition.disk_format.downcase)}' --size '#{definition.disk_size.to_i}' --format #{definition.disk_format.downcase}"
            shell_exec("#{command}")
          end
